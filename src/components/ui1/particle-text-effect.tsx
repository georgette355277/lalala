...
const DEFAULT_WORDS = ["Welcome", "to", "CoinMarketCap"]

export function ParticleTextEffect({ words = DEFAULT_WORDS, onComplete, onNext }: ParticleTextEffectProps) {
  ...
  const animate = () => {
    ...
    // Check if effect is complete and call onComplete after 3 seconds
    if (wordIndexRef.current === words.length - 1 && !hasCompletedRef.current) {
      // Wait 3 seconds after the last word is displayed
      if (frameCountRef.current % 480 === 180) { // 3 seconds at 60fps
    ...
    <div className="flex flex-col items-center justify-center w-full h-full bg-black">
      ...
      {showNextButton && onNext && (
        <div className="absolute bottom-40 left-1/2 transform -translate-x-1/2">
          <button
            onClick={() => onNext?.()}
            className="group relative grid overflow-hidden rounded-full px-6 py-2 transition-all duration-200 shadow-[0_1000px_0_0_hsl(0_0%_20%)_inset] hover:shadow-lg min-w-[150px] hover:scale-105 active:scale-95 text-lg font-semibold tracking-tight"
          >
